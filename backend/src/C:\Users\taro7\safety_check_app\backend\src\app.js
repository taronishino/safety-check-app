...
async function startServer() {
  ...
  const notificationService = new NotificationService(db.db);
  
  // 定期的な閾値チェック（30分ごと）
  setInterval(() => {
    logger.info('Running scheduled threshold check for notifications');
  ...
  // 初回チェック（5分後）
  setTimeout(() => {
    logger.info('Running initial threshold check for notifications');
    notificationService.checkAndNotify().catch(error => {
  ...
  app.listen(PORT, () => {
    ...
    logger.info('プッシュ通知サービスが有効になりました');
  });
}
